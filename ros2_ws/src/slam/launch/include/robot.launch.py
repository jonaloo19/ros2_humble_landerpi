...
def launch_setup(context):
    ...
    if compiled == 'True':
        ...
        peripherals_package_path = get_package_share_directory('peripherals')
        controller_package_path = get_package_share_directory('controller')
        slam_package_path = '/home/lonewolf/Desktop/ros2_ws/src/slam'

    # Robot description publisher for RViz visualization
    robot_description_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(slam_package_path, 'launch/include/robot_description.launch.py')),
        launch_arguments={
    ...
    # Only add robot_description for real robot (not simulation)
    # In simulation, spawn_model.launch.py already publishes robot_description
    if sim == 'false':
        launch_list.append(robot_description_launch)
