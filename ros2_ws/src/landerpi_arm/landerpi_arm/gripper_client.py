...
class GripperClient:
    ...
    def _send_gripper_command(self, position: float, duration: float = 1.0) -> bool:
        ...
        self.node.get_logger().info(f"  Waiting for gripper controller response (timeout={self.timeout}s)...")
        send_goal_future = self.action_client.send_goal_async(goal_msg)
        
        # Wait for goal acceptance
        rclpy.spin_until_future_complete(self.node, send_goal_future, timeout_sec=self.timeout)
        ...
        self.node.get_logger().info(f"  Waiting for execution to complete (timeout={self.timeout}s)...")
        rclpy.spin_until_future_complete(self.node, result_future, timeout_sec=self.timeout)
        
        if not result_future.done():
            self.node.get_logger().error(f"✗ Gripper execution TIMEOUT after {self.timeout}s")
            return False
